...
class MotorConfigApp:
    def __init__(self, root):
        ...
        # --- Variables: System Faults (Updated Keys) ---
        self.sys_faults = {
            "thermal_runaway": tk.BooleanVar(value=False),
            "overcurrent": tk.BooleanVar(value=False),
            "cell_short": tk.BooleanVar(value=False),
            "overvoltage": tk.BooleanVar(value=False),
            "locked_rotor": tk.BooleanVar(value=False),
            "overspeed": tk.BooleanVar(value=False)
        }
    ...
    def create_widgets(self):
        ...
        for key, var in self.sys_faults.items():
            # Using the exact dictionary key as the button text so it's clear what is being pushed
            cb = ttk.Checkbutton(sys_frame, text=key, variable=var, style="Toolbutton")
            cb.pack(fill="x", padx=40, pady=5)
        ...
        # Regen Toggle
        ttk.Checkbutton(motor_frame, text="Toggle Regen Active", variable=self.is_regen, style="Toolbutton").grid(row=5, column=0, columnspan=2, padx=40, pady=25, sticky="ew")
    ...
    def publish_loop(self):
        ...
        try:
            ...
            fault_payload = {
                key: var.get() for key, var in self.sys_faults.items()
            }
            
            # DEBUG PRINTS: Check your terminal/console to see if these trigger!
            print(f"Pushing to {self.topic_motor}: {motor_payload}")
            print(f"Pushing to {self.topic_fault}: {fault_payload}")
            print("-" * 40)
            ...
            # If something crashes, it will print here
    ...
    #     """Constructs the JSON and pushes to the respective MQTT topics every 1 second."""
    #     try:
    #         # 1. Prepare Motor Payload (Removed fault_type as it is now handled in system faults)
    #         motor_payload = {
    #             "speed_kmh": round(self.speed_kmh.get(), 2),
    #             "torque_nm": round(self.torque_nm.get(), 2),
    #             "rpm": round(self.rpm.get(), 2),
    ...
    #         # 2. Prepare System Faults Payload (Using exact dictionary keys)
    #         fault_payload = {
    #             key: var.get() for key, var in self.sys_faults.items()
    #         }
